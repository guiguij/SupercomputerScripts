...
$StarCcmRunVersionPath -rsh /opt/pbs/default/bin/pbs_tmrsh -batch $BatchFlagCommand -machinefile $PBS_NODEFILE -mpi platform  -batch-report -np $PBS_NCPUS -power -licpath 1999@81.134.157.100 -podkey $Podkey -classpath /scratch/uo95/gj5914/JarFilesPOI3.15ForBasicPPT/ $SimulationPath > ${PBS_O_WORKDIR}/sim.log &
while jobs %%
do
