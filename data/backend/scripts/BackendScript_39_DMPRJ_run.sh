...
$StarCcmRunVersionPath -rsh /opt/pbs/default/bin/pbs_tmrsh -batch $BatchFlagCommand -machinefile $PBS_NODEFILE -mpi platform  -batch-report -np $PBS_NCPUS -dmproject -dmnoshare -passtodesign "$PasstoDesign" -classpath /scratch/uo95/gj5914/JarFilesPOI3.15ForBasicPPT/ -renderthreads $PBS_NCPUS $SimulationPath > ${PBS_O_WORKDIR}/sim.log 
