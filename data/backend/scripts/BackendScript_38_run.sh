...
$StarCcmRunVersionPath -rsh /opt/pbs/default/bin/pbs_tmrsh -batch $BatchFlagCommand -machinefile $PBS_NODEFILE -mpi platform  -batch-report -np $PBS_NCPUS -power -licpath 1999@81.134.157.100 -podkey $Podkey -classpath /short/uo95/gj5914/JarFilesPOI3.15ForBasicPPT/ -renderthreads $PBS_NCPUS $SimulationPath > ${PBS_O_WORKDIR}/sim.log 
echo
echo "================ job done ==================="
echo "Date:   $(date)"
echo "retval: $retval"
