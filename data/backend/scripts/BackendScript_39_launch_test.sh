...
# Sb:          -q normal      m=$((30 * (i / 16)))  # 125GB and 64GB  available per node up to 1024 cores (64nodes)/  32GB above 1024 cores (64nodes)  see page 55 of C:\Users\Administrator\Dropbox\Trampo\IT\computers\Raijin\Using-Raijin1.pdf
  ...
  #If you will ask for a bit less, let say 30GB for normal queue and 125GB for normalbw queue, the job exceeding memory will be killed by PBS and you will get (in most cases  !) an error message
	...
	echo 1 Cores - 4 GB
		...
		qsub -N HRVST$i -q normal -lncpus=1 -e ./$i.err -o ./$i.out -lmem=4GB -lwalltime=00:05:00 -v StarCcmRunVersionPath="/scratch/uo95/gj5914/starccm/mixed/15.02.007/STAR-CCM+15.02.007/star/bin/starccm+",BatchFlagCommand="",LicensePath="1999@81.134.157.100",Podkey="k4j+t8IpNH57sxt54uWxxQ",SimulationPath="/g/data3/uo95/backend/data/customer_2568500248670/Synchronised_Folder/sim21/Cube.sim",CorePerNode="48" BackendScript_39_run_test.sh
